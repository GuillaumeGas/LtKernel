...
void create_process(void * task_addr, unsigned int size)
{
	...
	//	// On commence par nettoyer la page...
	//	mmset((u8*)p_new_code_page, 0, PAGE_SIZE);

	//	// Si on a de quoi copier sur une page entière, on fait ça sinon on copie seulement le reste de code à copier
	//	if ((size - count) < PAGE_SIZE)
	//		mmcopy((u8 *)task_addr + count, p_new_code_page, size - count);
	//	else
	//		mmcopy((u8 *)task_addr + count, p_new_code_page, PAGE_SIZE);
